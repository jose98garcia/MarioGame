...
class Controller implements ActionListener, MouseListener, KeyListener
{
	...
	void do_action(Action a)
	{
		...
		if(a == Model.Action.run)
		{
			mario.x += 10;
		}
		
		else if(a == Model.Action.jump)
		{
			mario.jump();
		}
		
		else if(a == Model.Action.run_and_jump)
		{
			model.mario.x += 10;
			model.mario.jump();
		}
	...
	*/
	...
	void update()
	{
		...
		double score_run_and_jump =
			model.evaluateAction(Model.Action.run_and_jump, 0);

		// Do the best one
		if(score_run_and_jump > score_jump &&
				score_run_and_jump > score_run)
